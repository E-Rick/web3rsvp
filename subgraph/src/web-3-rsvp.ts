...
export function handleNewEventCreated(event: NewEventCreated): void {
  // bytes32 means you convert the id to a hexadecimal string rep the bytes in the array
  let newEvent = Event.load(event.params.eventID.toHex());

  // For each mapping function load the entity with a unique id and create new instance only if that result is null
  // the smart contract emits eventID and 
  if (newEvent == null) {
    ...
    // use the web3.storage and access the data stored there.
    let metadata = ipfs.cat(event.params.eventDataCID + "/data.json")

    // You want to better explain what cat does and the purpose of the metadata here. 
    if (metadata) {
